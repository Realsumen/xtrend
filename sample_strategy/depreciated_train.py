...
from depreciated_loss_functions import *
from model import *
...
files = [f"future_data/{f}" for f in os.listdir(folder_path) if f.endswith(".csv")][:10]
...
def run():
    ...
    data_list = process_data_list(
        files, macd_timescales, rtn_timescales, last_date=20211231
    )
    ...
    target_set, labels, map = generate_tensors(
        data_list, timesteps, encoder_type="one-hot", return_map=True
    )
    ...
    # 初始化模型
    xtrend_model = ModelWrapper(
        features_len, hidden_dim, encoding_size, num_heads=4, dropout_rate=0.4
    )
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        train_dataset = (
            dataset.take(train_size).shuffle(buffer_size=10000).batch(batch_num)
        )
        ...
        print(
            f"Epoch {epoch+1}/{num_epochs}, Validation Joint Loss: {val_joint_loss:.2f}, Validation MLE: {val_mle:.2f}, Validation Sharpe: {val_sharpe:.2f}"
        )
